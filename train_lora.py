...
from models.get_models import print_trainable_parameters, get_tokenizer, get_prefix_tuning_models, get_lora_models, get_fft_models
import argparse
from customized_trainer import customized_trainer

parser = argparse.ArgumentParser()
parser.add_argument('--peft_type', type=str,
                    choices=['prefix', 'lora', 'fft'])
...
parser.add_argument('--output_folder', type=str, default='results_lora')
...
if args.ckpt is not None:
    ...
    if os.path.exists(output_jsonl):
        ...
        args.r = dict_args.get('r', args.r)
        args.lora_alpha = dict_args.get('lora_alpha', args.lora_alpha)
        args.lora_dropout = dict_args.get('lora_dropout', args.lora_dropout)
        args.bias = dict_args.get('bias', args.bias)
...
if peft_type == 'lora':
    exp_name = exp_name + f'r={args.r}-'
    exp_name = exp_name + f'alpha={args.lora_alpha}-'
    exp_name = exp_name + f'dropout={args.lora_dropout}-'
    exp_name = exp_name + f'bias={args.bias}-'
...
if peft_type == 'lora':
    model, tokenizer, model_config = get_lora_models(load_bit=args.load_bit,
                                                     ...
                                                     r=args.r, target_modules=args.target_modules,
...
if args.ckpt is not None:
    ...
    # if not (args.compute_rank or peft_type == 'fft'):
